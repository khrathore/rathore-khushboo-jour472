...
vendor_payments <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2023.csv", guess_max = 322138) %>% 
  clean_names()

# Load in the Maryland grants and loan data
grants_loans <- read_csv("data/State_of_Maryland_Grant_and_Loan_Data__FY2009_to_FY2022.csv", guess_max = 17765) %>% 
  clean_names()
```
...
vendor_payments <- vendor_payments %>% 
  mutate(pgcs = case_when(
    (str_detect(vendor_name, "PG")  & str_detect(vendor_name, "SCHOOL")) ~ "yes",
    (str_detect(vendor_name, "PRINCE GEORGES") & str_detect(vendor_name, "SCHOOL"))~ "yes",
     TRUE~"no"
  ))

pg_schools <- vendor_payments %>% 
  filter(pgcs == "yes")
...
A2:
2021 has the largest total amount. There is no data for 2011, 2012, 2016, or 2022.
```{r}
pg_schools_fy <- pg_schools %>% 
...
A3.
A&I  INC, A&I INC. and A & I INC should all be combined.
AEROSOL MONITORING & ANALYSIS INC and AERSOL MONITORING & ANALYSIS INC should be combined.
GLOBAL EQUIPMENT CO, GLOBAL EQUIPMENT COMPANY, and GLOBAL EQUIPMENT COMPANY INC should be combined.
Unfortunately, some of these don't necessarily have the same zip code.I would probably start standardizing by changing all abbreviations (like Inc, Assoc, etc.) to their longest form. Then, I would remove or standardize the spaces and punctuation. I would next combine things by like zip codes.
```{r}
umd_vendors <- vendor_payments %>% 
...
A4:
STEM cell research funding gradually decreased until about 2019, when the numbers started increasing again, excepting 2021.
```{r}
