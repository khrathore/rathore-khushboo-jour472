...
write_csv(matches_compares, "data/home_away_comp.csv")
...
by_mascot <- mascot_list %>% 
  ...
  summarize(
    mean_fouls = mean(avg_fouls),
    med_fouls = median(avg_fouls),
    mean_goals = mean(avg_goals),
    med_goals = median(avg_goals),
    med_shut = median(avg_shutouts)
  )
