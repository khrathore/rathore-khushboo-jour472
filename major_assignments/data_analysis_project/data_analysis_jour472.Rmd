...
We found that the amount of people with those numbers who score game winning goals isn’t as frequent as we’d suspected. Instead, we found a small trend of girls with higher random numbers, such as 66 or 52. We often attributed this back to a parent playing professionally (usually football) and that being their number. However, one of the girls on UCLA (Sunshine Fontes, #50) was one of these players who had the most game winning goals. Though she didn’t have a record of professional athlete parents, she has had time on the US national team. If we were to pursue this idea, it would be interesting to see if we could find how many collegiate players are also National team players and where most of them go to school. I’d want to see if the team with the most national team players (for any country) has one of the best records. 
...
jersey_chart <- player_by_jersey %>% 
  ggplot(aes(x = num_players, y = median_goals,label = jersey)) +
...
# Shouldn't percent_start be the percentage of games started/games played (instead of total games the team plays), based on the theory that you can't start a game you don't play in? Seems like otherwise you're even more likely to have seniors dominate the starting percentages. 

# Make this started/games ELIGIBLE (games from seasons they competed in)
