...
mascot_matches <- match_all %>%
  select(team, team_id, opponent, opponent_id) %>% 
  mutate(team_id = as.character(team_id)) %>% 
  left_join(team_crosswalk, by = c("team", "team_id")) %>% 
  select(team, team_id, opponent, opponent_id, mascot) %>% 
  rename(mascot_team = mascot) %>% 
  left_join(team_crosswalk, by = c(""))
