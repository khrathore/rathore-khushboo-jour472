...
team_links <- team_links_2020 %>% 
  ...
  mutate(team_number = str_extract(playerstatsurl, "(?<=team/).*?(?=/stats)"))
...
team_crosswalk <- all_teams %>%
  mutate(team_number = str_extract(url, "(?<=team/).*?(?=/)")) %>% 
  distinct(institution, conference, season, .keep_all = TRUE) %>% 
  left_join(team_links, by = "team_number") %>% 
  left_join(mascot_crosswalk, by = c("school" = "team")) %>% 
  select(institution, mascot, school, conference) %>% 
  rename(team = school) %>% 
...
tc_season <- team_crosswalk %>% 
  ...
  distinct(institution, conference, season, .keep_all = TRUE) %>% 
  select(institution:conference, season)
```
...
aac_chart <- stats_aac %>% 
  ...
  geom_point(
    data=inner_join(stats_aac, top_shut_nogoals), 
    aes(x = avg_goals, y = avg_shut, size = win_percent), 
    color="red",
    alpha = .5
    ) + 
  geom_point(
    data=inner_join(stats_aac, top_goal_noshut), 
    aes(x = avg_goals, y = avg_shut, size = win_percent), 
    color="blue",
    alpha = .5
    ) + 
  scale_size(range = c(1, 8), name="Win Percent") +
  geom_vline(xintercept = all_conf$goal_avg[1]) + 
  geom_hline(yintercept = all_conf$shut_avg[1]) +
  geom_text_repel(
    aes(x=avg_goals, y=avg_shut, label=paste(team, "-", win_percent))
  ) +
  labs(
    x = "Mean Goals",
    y = "Mean Shutouts"
