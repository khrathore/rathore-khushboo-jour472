...
**Answer**
Yes. They don't have the same values, but the structures are the same.
```{r}
...
**Answer** 
There has been growth in party registration overall, but it seems like the increase in Democrats has been significantly larger.
```{r}
...
**Answer**
There are 24 rows, which is how many there should be. 
```{r}
...
**Answer** 
It contains population by race and how many are adults. 
```{r}
...
**Answer**
Prince George's County, which has 77.1% Democrats.
```{r}
...
**Answer**
Howard County, which has 52.57% Democrats. The highest percentage of Asians is much lower than the highest percentage of Black population, but the democrat percentage is not as different.
```{r}
maryland_population_with_voters_and_demographics %>%
