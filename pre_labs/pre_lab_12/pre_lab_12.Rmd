...
**Answer**
The dataframe is the same one we created in the last lab, but with text added. Basically, it accounts for the 999 most recent releases by giving the date, title, url, and text from the release. 
```{r}
...
**Answer**
Unique makes a 1D list of each distinct element. However, there is no distinction of uppercase and lowercase, and it includes the comma.
```{r}
...
**Answer**
It is a list of all the unique words in all of Cardin's press releases.
```{r}
...
**Answer**
The graphic shows, in descending order, the top 25 words used in Cardin's press releases. Not really interesting, most of them are filler words, so it would make more sense to remove them.
...
**Answer**
This is definitely more interesting than before. It shows 50 words that he uses often, indicating that he talks a lot about health, art, and funding for programs. However, we can't judge the context of these based on the data we have.
```{r}
unique_words %>%
...
**Answer**
Three dataframes that show the most often used unique words in 2021, 2022, and 2023. High up on the lists across these years is health.
```{r}
unique_words_2021 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
The top ten most commonly used two-word phrases in the speeches, just in 2021. It includes many references to COVID and health.
```{r}
releases %>%
  filter(date < '2022-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
In 2022, there is a lot of location phrases. This isn't really helping much, unfortunately.
```{r}
releases %>%
...
**Task** Run the following code and describe the results.
**Answer**
Another dataframe, this one representing the 2023 release text, and shows a lot more information about human rights and legal decisions made by the government.
```{r}
releases %>%
  filter(date >= '2023-01-01') %>%
  unnest_tokens(bigram, text, token = "ngrams", n = 2) %>%
...
**Answer**
I think that in certain contexts, critical, emergency, risk, and crisis can be part of positive sentiments. Additionally, trump seems kind of counter intuitive.
```{r}
