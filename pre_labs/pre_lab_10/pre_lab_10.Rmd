...
**Answer**
The glimpse of this data shows the geometric makeup of the map that we are trying to recreate. The columns, listed below, show various areas and their bounding boxes that will come together to create one map. The geometry column is also of type "MULTIPOLYGON"
OBJECTID
ZIPCODE2
ZIPNAME
EXISTING
...
**Answer**
There are area names, main, five digit zip codes and the zip code that follows the dash. There is also information about the size and boundaries of each zip code.
```{r}
View(md_zips)
```
...
**Answer**
It is a map of Maryland with the longitude and latitude of the state and with the boundaries of each zip code overlayed on the state.
```{r}
md_zips %>%
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The data shows 18 columns, mostly of characters that show different coordinates and that shows the geometry and location of each county.
```{r}
counties <- counties()
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We get a list of all the counties in Maryland and their information.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output shows borders of each county, but it's also... not great because it doesn't account for the fact that some counties cover the bay.
```{r}
md_counties %>%
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This shows the zip codes over top of the counties, which changes the shape a little bit, but the counties still include the bay.
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer** 
The output is a dataframe that shows each zip code and the number of foreclosure notices in that zip
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This creates a dataframe that joins the number of foreclosure notices with information about the geography of each zip code
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The same graph as before, but with shading to indicate the number of foreclosures in the zip code. The map also removes the water boundaries
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We get the population in each county in the state of Maryland in a dataframe. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This gives a dataframe of the populations combined with the geographical boxes for each.
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              year = 2021,
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Two dataframes, one of the population/geometry info and one of the foreclosure notices by county.
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A dataframe that shows the county, population and the geometry of the county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A dataframe that joins the population data with the numbers of foreclosures in the county
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
RATES!!!! FORECLOSURE NOTICES PER 1000 PEOPLE ADDED TO DF
```{r}
md_pop_with_foreclosures <- md_county_population %>%
  left_join(md_county_notices, by='county') %>% 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Graaaaph with data by county of the foreclosure rates.
```{r}
ggplot() +
...
**Answer**
The northeast has more notices, as does the southern parts of Maryland. It's hard to really quantify this as regional.
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
