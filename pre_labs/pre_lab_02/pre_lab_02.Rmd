...
**Answer**
We can do other analysis on the journalism_courses dataframe and are not erasing data, just getting a new subset of it.
...
**Answer:**
The column type is now date, and they are formatted as YYYY-MM-DD
...
**Answer**
The month column is all dates, each representing the first day of a month. I believe that each row is that entire month (2022-06-01 would be all the days in June 2022) and the contributions that were made over that month.
...
**Answer**
This code is creating two columns, one that is the total number of votes in the county and the other one calculated the percentage of the county's votes that Moore received as a decimal.
...
**Answer**
After being changed into percentages, the counties are now arranged by the percent of their vote that went to Wes Moore, from highest to lowest percent.
...
**Answer**
I don't think it's more useful, but I think it's interesting (but not surprising) to see that more rural areas had fewer voters for Moore, and the lowest percent was 22, which is still pretty significant. However, the other table gives the same information, even if it is in a different order.
...
**Answer**
str_squish will get rid of white spaces at the beginning and end of a given string
str_to_title will capitalize the first letter of each word.
...
**Answer**
The statement is looking at each address and testing to see if one of the cases (aka a state name) is in it. If there is, the state abbreviation is put into a new column of the dataframe called "state"
```{r}
...
**Answer**
For the most part it does make sense. Maryland and Virginia and pretty local, and Amazon is based in Washington. Those are much more significant than any other states, which could contain small businesses or outside contracts.
...
1. Was there a significant difference in class seats available during the pandemic, when courses were largely online?

2.  How many different organizations per state recieved money from the Maryland campaigns?

3.  When were the majority of expenditures happening in relation to voting periods?
