...
**Answer**
55,632 observations, 7 variables
...
**Answer**
Line 1: Tells R that we are doing things to the umd_courses dataframe
Line 2: Asks R to create buckets/groups by the term that each course is offered in
Line 3 & 4: Create a summary table of the number of courses in each term
...
**Answer**
A new line was added that sorted the table by the number of courses offered from most to least
...
**Answer**
The data is now grouped and counts the number of courses by two variables, term and department. Then, it is sorted first by term, and then by the name of the department. There was also an R Console output that said "`summarise()` has grouped output by 'term'. You can override using the `.groups` argument."
...
Compared to the previous codeblock, what has changed and how did that affect the result? 
**Answer**
The data is now grouped by term, with two columns that indicate the number of courses offered and the number of seats offered that term.The data is sorted from most seats available to least seats available.
...
**Answer**
The table also had data on mean seats offered each term and median seats offered each term, then the table is sorted by most total seats to least. The top terms all end with 08, meaning that they are all Fall term courses.
...
**Answer**
The code now groups by departments and adds rows for the minimum amount of seats for a course in that department and the max amount of seats for a course in that department. It is again sorted by which department has the most total seats.
...
1. Why are there more seats during fall semesters than other semesters?

2. Why are the means and medians of seats so different when sorted by term and by department?

3. Do courses tend to have the same amount of seats each semester? How much does the seat availability vary and can this indicate growth or decline of certain majors?
