...
**Answer**
The dataframe shows a list of states, who won that state, the percentage of Trump votes in that state, and data on PPP loans given in those states
```{r}
...
**Answer**
The plot shows a generally positive correlation in percent of trump voters and percent of PPP loans. There are outliers, as expected, but it is a usually positive trend.
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
There is a less positive trend with a more significant spread in this chart. While there may still be a relationship, it isn't nearly as strong.
```{r}
...
**Answer**
The table shows a relatively strong positive correlation between Trump voter percent and percent of businesses receiving PPP loans. There is a VERY small positive correlation between Trump voter percent and $ per employee.
```{r}
reveal_data %>%
...
**Answer**
The model shows that the finding is definitely statistically significant.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
This shows that the finding is not statistically significant since the p-value is above .05
```{r}
...
**Answer**
The table shows the mean of how much money per employee was given to states depending on whether they voted more for Clinton or Trump. There is a ~$300 per employee difference.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
The t-test results indicate that there probably is not a large difference, as the p-value is .23.
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
The percentages show that there is some difference, one that is significant in how often Clinton states and Trump state businesses recieved the loans.
```{r}
