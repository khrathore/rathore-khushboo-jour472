...
https://www.datawrapper.de/_/j6LE4/
...
md_gov_county <- md_gov_county %>%
  mutate(tot_votes = Cox + Moore + Lashar + Wallace + Harding + `Write-ins`) %>% 
  mutate(pct_moore = Moore/tot_votes*100)
...
The graph now only shows Maryland instead of showing the entire country.
...
**Answer** Describe the changes from the previous map
Now, when we click on or hover over the different areas of the map, we see the county name and the percent of votes in the county that went to Moore, but nicely rounded.
...
https://www.datawrapper.de/_/Phzpt/