...
**Answer**
Line 1: Create a new dataframe based on the original data
Line 2: Create a new column "month" that shows the month, from the date column (as its abbreviation)
Lines 3,4,5: Count the number of calls in each months and sort from most to least calls.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
A chart that shows bars for each month with heights that indicate the number of calls. The axis are labelled.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
...
**Answer**
The columns are now in height order, where it goes from fewest to most calls. The code has added more to the aesthetic section and made the x-axis reordered by total_calls.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The chart has been flipped so that the months are on the y-axis and the count is on the x-axis. This was done using the function coord_flip
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The chart has inverted the background colors so that the base is white with gray gridlines. This was done using theme_minimal()
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Answer**
Quiet frankly: It's unglier. But the main difference is that there are no vertical grid lines and a blue background with a white line marking each month horizontally.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The chart now has labels, including a title, x-axis label, y-axis label, and a sourcing caption, accomplished using the labs() function
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I see a table of calls by date, organized by date, going from January to December.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Spikes. Many, many spikes. It shows the number of calls per day in the year.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Um the same chart but now I cannot actually read the dates.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I can kind of see the dates, but they still aren't super clear and easy to read.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
