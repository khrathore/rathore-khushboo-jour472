...
When I follow the instructions, I see that some of the rows have an unexpected value, that is not a double, in column 4.
```{r}
...
**Answer**
It doesn't have parsing errors, but it has problems with column specification and the fact that the seperator was not defined.
```{r}
...
**Answer**
547 distinct zip codes. It is likely that some of the four digit codes have dropped a 0 in front, and some of the codes with letters and numbers are from foreign countries.
```{r}
payments %>%
  group_by(`Vendor Zip`) %>%
...
**Answer**
There are multiple NAs, one of the 2013 entries is just "L", 2017 has a random Contract.
```{r}
md_grants_loans %>% 
...
**Answer**
There are very, very few values under $1,000, only 25 grants were given under that value out of 17,765.
```{r}
md_grants_loans %>% 
  arrange(Amount)
```
...
The headers become all lowercase, and any spaces become underscores.
```{r}
# cleaning function
...
There are 50 entries that have at least one duplicate, which means there are at least 25 payments.
```{r}
cleaned_md_grants_loans %>%
  get_dupes()
```
...
**Answer**
The dataframe now has 17,740 entries.
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans %>%
  clean_names() %>% 
...
**Answer**
The column is now ONLY values of five characters, the first five of the whole string.
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans %>%
  clean_names() %>% 
...
21202, which is in Baltimore. That makes sense to me since Baltimore has the aquarium and the harbor, which are pretty significant locations in Maryland. It's also on a main highway, so many people pass through there and it would make more sense for the state to give them money.
```{r}
cleaned_md_grants_loans %>% 
